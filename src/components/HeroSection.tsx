...
const HeroSection = () => {
  ...
  return (
    <section className="relative min-h-screen flex items-center bg-gradient-hero overflow-hidden pt-20 pb-12 md:pt-16">
        ...
        >
          <div className="flex flex-wrap gap-2 md:gap-3">
            {badges.map((b, i) => (
              <motion.span
                ...
                className="inline-flex items-center gap-1.5 px-2.5 py-1 md:px-3 md:py-1.5 rounded-full bg-card border border-border text-[11px] md:text-xs font-medium text-muted-foreground glow-primary"
              >
                <b.icon className="h-3 w-3 md:h-3.5 md:w-3.5 text-primary" />
                {b.label}
              </motion.span>
            ))}
          </div>
          ...
          <div className="flex flex-wrap gap-3 md:gap-4">
            <Button size="lg" className="glow-primary" asChild>
              <a href="#products" onClick={scrollTo('#products')}>{t('Explore Products', 'اكتشف المنتجات')}</a>
            </Button>
            <Button variant="outline" size="lg" asChild>
              <a href="#contact" onClick={scrollTo('#contact')}>{t('Book Consultation', 'احجز استشارة')}</a>
            </Button>
          </div>
          ...
          <div className="flex gap-6 md:gap-8 pt-2">
            {[
              ...
              <div key={i} className="text-center">
                <div className="font-display text-xl md:text-2xl font-bold text-primary">{s.val}</div>
        ...
        >
          <div className="relative">
            ...
            <img
              ...
              width={800}
              height={1024}
              className="relative rounded-2xl shadow-2xl max-h-[350px] sm:max-h-[450px] lg:max-h-[600px] object-cover glow-primary"
            />
          </div>
        </motion.div>
      </div>
